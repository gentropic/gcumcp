// End-to-end smoke test for @gcu/numen: spin the bridge in an isolated $HOME,
...
// Isolated HOME so we don't touch the real ~/.gcu/numen.json.
const tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'numen-'));
...
try {
  ...
  assert.ok(names.includes('numen_listClients') && names.includes('numen_getConnectionInfo'), 'built-in tools present');
  ...
  assert.ok(staticNames.includes('numen_listTools') && staticNames.includes('numen_callTool'), 'listTools/callTool are static built-ins');
  const lt = JSON.parse((await mcp('tools/call', { name: 'numen_listTools', arguments: {} })).result.content[0].text);
  ...
  const ct = await mcp('tools/call', { name: 'numen_callTool', arguments: { name: 'echo', arguments: { text: 'via dispatch' } } });
  ...
  const bad = await mcp('tools/call', { name: 'numen_callTool', arguments: { name: 'nope' } });
  ...
  const tokFile = path.join(tmpHome, '.gcu', 'numen.json');
