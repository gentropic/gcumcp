...
`@gcu/numen` is the **single, official way to connect GCU browser surfaces to
...
|---|---|---|---|
| `numen-bridge.js` | this repo (node, dev-time) | MCP stdio ↔ WS/HTTP relay, tool merge, routing | Yes |
...
> *fetch* broker (a Chromium extension). That is **not** this. This (`@gcu/numen`)
...
3. **Add `.mcp.json`** to the app's repo:
   ...
   { "mcpServers": { "webmcp-weir": { "command": "node",
     "args": ["numen-bridge.js", "--app", "weir", "--port", "7801"] } } }
   ```
   (Point `command`/`args` at wherever the bridge lives — a sibling clone, an
   `npx @gcu/numen`, or a vendored copy.)
4. **Connect once.** `node numen-bridge.js --app weir --port 7801 --info` prints
...
- Point its `.mcp.json` at this `numen-bridge.js` with `--app auditable --port
  7802`.
...
- **User-scope install.** *Resolved by the `fs` transport + distribution.* The bridge
  is launched per-surface via `npx -y github:gentropic/numen --app <x> --transport fs`
  ...
  jsr:@gcu/numen …` (versioned, key-free). (Future: `npx @gcu/numen` once on npm.)
